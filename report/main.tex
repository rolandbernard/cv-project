...
The resulting system was evaluated on five video sequences from the CMU Panoptic dataset, benchmarking the performance impact of different 2D pose estimation model capacities and varying camera viewpoint counts. Computational runtime analysis shows that the 2D detection phase dominates the processing, consuming between 64\% and 91\% of per-frame execution time and scaling linearly with the number of camera feeds. Furthermore, while overall Multi-Object Tracking Accuracy (MOTA) remains relatively stable at around 0.95 across configurations, increasing the viewpoint density improves 3D reconstruction precision. For the smallest model configuration, scaling from 4 to 16 cameras reduces the Mean Per-Joint Position Error (MPJPE) from 5.44 cm down to 5.44 cm. Similarly, increasing the backbone model capacity under a fixed camera count yields tracking error reductions.
\end{abstract}
