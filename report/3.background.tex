...
Even once corresponding points across multiple camera views have been identified, their 3D position $\tilde{x}_w$ must still be reconstructed from the 2D pixel locations. Rays back-projected from pixels will nearly never intersect perfectly in 3D space due to measurement noise. As such, a noise robust algorithm must be used that can recover 3D coordinates using multiple noise 2D projections \parencite[p. 16-17]{Lanz2026Notes}. If we denote $\tilde{x}_i^s$ the homogeneous 2D observation in the $i$-th camera, and $\mathbf{P}_i$ the projection matrix of that that camera, because $\tilde{x}_i^s$ and the projected point $\mathbf{P}_i \tilde{x}_w$ represent the same point up to scale, their cross product must be zero:
\begin{align*}
