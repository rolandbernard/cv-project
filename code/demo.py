...
class LiveSkeletonPlayer:
    """A class to visualize tracked skeletons in 3D using PyVista."""

    def __init__(self, cameras: list[Camera], center=(0, 0, 0), up=(0, -1, 0)):
        cam_dicts = []
        for cam in cameras:
            cam_dicts.append({
                "R": cam.rotation.tolist(),
                "t": cam.translation.tolist(),
    ...
    def setup_cameras(self, cameras, scale=0.1):
        ...
        for cam in cameras:
            ...
            self.pl.add_mesh(
                pv.PolyData(vertices, lines=lines),
                color="black",
                line_width=2
            )
            self.pl.add_mesh(
                pv.Sphere(radius=z_cam * 0.1, center=camera_center),
                color="red"
            )
    ...
    def update(self, tracks):
        """Update the 3D visualization with new track data."""
        for actor in self.track_actors.values():
            actor.SetVisibility(False)
            
        for track in tracks:
            kpts = np.array(track["kpts"])
            new_mesh = pv.PolyData(kpts)
...
def estimate_camera_params(img1, img2, distance=None, fov1=None, fov2=None):
    ...
    def epipolar_loss(params):
        ...
        K1_c = np.array([[f1_c, 0, cx1_c], [0, f1_c, cy1_c], [0, 0, 1]], dtype=np.float32)
        K2_c = np.array([[f2_c, 0, cx2_c], [0, f2_c, cy2_c], [0, 0, 1]], dtype=np.float32)
    ...
    def get_ba_errors(params, pts1, pts2):
        ...
        K1_ba = np.array([[f1_ba, 0, cx1_ba], [0, f1_ba, cy1_ba], [0, 0, 1]], dtype=np.float32)
        K2_ba = np.array([[f2_ba, 0, cx2_ba], [0, f2_ba, cy2_ba], [0, 0, 1]], dtype=np.float32)
        ...
        imgpts1, _ = cv2.projectPoints(pts3d, np.zeros(3), np.zeros(3), K1_ba, None)
    ...
    res_ba = opt.least_squares(
        get_ba_errors,
        np.concatenate([[f1, cx1, cy1, f2, cx2, cy2], rvec_init.flatten(), t.flatten()]),
...
def estimate_camera_params_moge(img1, img2, distance=None):
    ...
    def infer(img):
        t_img = torch.tensor(img / 255.0, dtype=torch.float32, device=device).permute(2, 0, 1)
        with torch.no_grad():
    ...
    if distance is not None and cb > 1e-6:
        ...
        pm1 *= scl
        pm2 *= scl
        print(f"MoGe Baseline: {cb:.2f}m. Scaled to user distance: {distance:.2f}m.")
    else:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Cross-view 3D skeleton tracking demo.")
    ...
    parser.add_argument("--distance", type=float, help="Known distance between cameras for scaling")
    parser.add_argument("--fov1", type=float, help="Field of view for camera 1 (degrees)")
    parser.add_argument("--fov2", type=float, help="Field of view for camera 2 (degrees)")
    parser.add_argument("--resize", type=int, nargs=2, help="Resize frames to (width, height)")
    parser.add_argument("--moge", action="store_true", help="Use MoGe-2 for camera estimation")
    parser.add_argument("--cross_first", action="store_true", help="Use CrossViewFirstTracker")
    ...
    try:
        if args.moge:
            ...
            K1, K2, R, t = estimate_camera_params(img1, img2, args.distance, args.fov1, args.fov2)
    ...
    detector = PoseDetector(path="./nets")
    physics = build_constrained_physics(scale=1.0)
    ...
    try:
        while True:
            ...
            tracks_data = [
                {"id": t.id, "kpts": t.get_keypoints().tolist()}
                for t in tracker.get_prediction()
            ]
            player.update(tracks_data)
            
            stacked = np.vstack([cv2.cvtColor(f.cpu().numpy(), cv2.COLOR_RGB2BGR) for f in frames])
            if stacked.shape[0] > 800:
                h_new = 800
                w_new = int(stacked.shape[1] * 800 / stacked.shape[0])
